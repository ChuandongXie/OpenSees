/* ****************************************************************** **
**    OpenSees - Open System for Earthquake Engineering Simulation    **
**          Pacific Earthquake Engineering Research Center            **
**                                                                    **
**                                                                    **
** (C) Copyright 1999, The Regents of the University of California    **
** All Rights Reserved.                                               **
**                                                                    **
** Commercial use of this program without express permission of the   **
** University of California, Berkeley, is strictly prohibited.  See   **
** file 'COPYRIGHT'  in main directory for information on usage and   **
** redistribution,  and for a DISCLAIMER OF ALL WARRANTIES.           **
**                                                                    **
** Developed by:                                                      **
**   Frank McKenna (fmckenna@ce.berkeley.edu)                         **
**   Gregory L. Fenves (fenves@ce.berkeley.edu)                       **
**   Filip C. Filippou (filippou@ce.berkeley.edu)                     **
**                                                                    **
** ****************************************************************** */
                                                                        
// $Version: 1.0 $
// $Date: 2022-03-25 14:32:14
// $Source: /usr/local/cvs/OpenSees/SRC/material/uniaxial/SPSW03.cpp,v $

// Written: Chuandong Xie
//			Xi'an University of Architecture and Techonology 
// Create: 2022-03-25 14:32:14
// Mail: chuandongxie@xauat.edu.cn
// 
// Description: This file contains the class implementation for SPSW03.
//				SPSW03 is developed based on SPSW02 but considers 
//				imperfection of infill plates
//-----------------------------------------------------------------------
//                  SPSW03 by Chuandong Xie (2022)
//-----------------------------------------------------------------------

#include <elementAPI.h>
#include "SPSW03.h"
#include <MaterialResponse.h>

#include <Vector.h>
#include <Channel.h>
#include <math.h>
#include <Information.h>
#include <stdlib.h>
#include <float.h>
#include <OPS_Globals.h>


#ifdef _USRDLL
#define OPS_Export extern "C" _declspec(dllexport)
#elif _MACOSX
#define OPS_Export extern "C" __attribute__((visibility("default")))
#else
#define OPS_Export extern "C"
#endif

static int numSPSW03 = 0;
// UniaxialMaterial SPSW03 tag E0 b <-geom Fpy t h l> 
//                  <-params Fts Fcs cmpUnldngEFac sigTEfac sigTFfac epsTFfac> 
//                  <-R $R> <-damage epsPCFac pstCapEFac gama c resFac> 
//                  <-IPF mode impf>;

void*
OPS_SPSW03()
{
	// Pointer to a uniaxial material that will be returned
	UniaxialMaterial* theMaterial = 0;

	int tag;
	double E0, b;
	double fpy, nFac, t, hs, l;
	double Fts, Fcs, cmpUnldngEFac, sigTEfac, sigTFfac, epsTFfac;
	double R;
	double epsPCFac, pstCapEFac, gama, c, resFac;
	int mode;
	double impf;

	//double lStrip, lImpf, epsImpf, E0Impf;

	//opserr << "E0Impf at BEG = " << E0Impf << endln;
	
	bool paramsSet = false;
	int argc = OPS_GetNumRemainingInputArgs();
	int numData = 1;
	int curArg = 2;
	if (OPS_GetIntInput(&numData, &tag) != 0) {
		opserr << "WARNING invalid -tag ";
		opserr << "uniaxialMaterial SPSW03: " << tag << endln;
		return 0;
	}
	curArg ++;

	if (OPS_GetDoubleInput (&numData, &E0) != 0) {
		opserr << "WARNING invalid -E0 ";
		opserr << "uniaxialMaterial SPSW03: " << tag << endln;
		return 0;
	}
	curArg ++;

	if (OPS_GetDoubleInput (&numData, &b) != 0) {
		opserr << "WARNING invalid -b ";
		opserr << "uniaxialMaterial SPSW03: " << tag << endln;
		return 0;
	}
	curArg ++;

	const char* str = OPS_GetString();
	curArg ++;

	fpy = 0;
	if (strcmp(str , "-geom") == 0)
	{
		//Fpy nFac t h l
		if (OPS_GetDoubleInput (&numData, &fpy) != 0) {
			opserr << "WARNING invalid -Fpy ";
			opserr << "uniaxialMaterial SPSW03: " << tag << endln;
			return 0;
		}
		curArg ++;

		if (OPS_GetDoubleInput(&numData, &nFac) != 0) {
			opserr << "WARNING invalid -nFac ";
			opserr << "uniaxialMaterial SPSW03: " << tag << endln;
			return 0;
		}
		curArg++;

		if (OPS_GetDoubleInput (&numData, &t) != 0) {
			opserr << "WARNING invalid -t ";
			opserr << "uniaxialMaterial SPSW03: " << tag << endln;
			return 0;
		}
		curArg ++;

		if (OPS_GetDoubleInput (&numData, &hs) != 0) {
			opserr << "WARNING invalid -h ";
			opserr << "uniaxialMaterial SPSW03: " << tag << endln;
			return 0;
		}
		curArg ++;

		if (OPS_GetDoubleInput (&numData, &l) != 0) {
			opserr << "WARNING invalid -l ";
			opserr << "uniaxialMaterial SPSW03: " << tag << endln;
			return 0;
		}
		curArg ++;
	} else if (strcmp(str , "-params") == 0)
	{
		paramsSet = true;
		//Fts, Fcs, cmpUnldngEFac, sigTEfac, sigTFfac, epsTFfac
		if (OPS_GetDoubleInput (&numData, &Fts) != 0) {
			opserr << "WARNING invalid Fts ";
			opserr << "uniaxialMaterial SPSW03: " << tag << endln;
			return 0;
		}
		curArg ++;

		if (OPS_GetDoubleInput (&numData, &Fcs) != 0) {
			opserr << "WARNING invalid Fcs ";
			opserr << "uniaxialMaterial SPSW03: " << tag << endln;
			return 0;
		}
		curArg ++;

		if (OPS_GetDoubleInput (&numData, &cmpUnldngEFac) != 0) {
			opserr << "WARNING invalid cmpUnldngEFac ";
			opserr << "uniaxialMaterial SPSW03: " << tag << endln;
			return 0;
		}
		curArg ++;

		if (OPS_GetDoubleInput (&numData, &sigTEfac) != 0) {
			opserr << "WARNING invalid sigTEfac ";
			opserr << "uniaxialMaterial SPSW03: " << tag << endln;
			return 0;
		}
		curArg ++;

		if (OPS_GetDoubleInput (&numData, &sigTFfac) != 0) {
			opserr << "WARNING invalid sigTFfac ";
			opserr << "uniaxialMaterial SPSW03: " << tag << endln;
			return 0;
		}
		curArg ++;

		if (OPS_GetDoubleInput (&numData, &epsTFfac) != 0) {
			opserr << "WARNING invalid epsTFfac ";
			opserr << "uniaxialMaterial SPSW03: " << tag << endln;
			return 0;
		}
		curArg ++;
	}
	if (fpy == 0 && !paramsSet) {
		opserr << "WARNING at least one of -params or -geom options must be provided ";
		opserr << "uniaxialMaterial SPSW03: " << tag << endln;
		return 0;
	}
	if (fpy != 0 && paramsSet) {
		opserr << "WARNING both -params and -geom options cannot be used at the same time ";
		opserr << "uniaxialMaterial SPSW03: " << tag << endln;
		return 0;
	}
	
	R = 50;
	if (argc >= curArg + 1)
	{
		str = OPS_GetString();
		curArg ++;
		if (strcmp(str , "-R") == 0)
		{
			if (OPS_GetDoubleInput (&numData, &R) != 0) {
				opserr << "WARNING invalid -R ";
				opserr << "uniaxialMaterial SPSW03: " << tag << endln;
				return 0;
			}
			curArg ++;
		}
	}

	epsPCFac = 1.e20;
	pstCapEFac = b;
	gama = 10000;
	c = 1;
	resFac = 0.001;
	if (argc >= curArg + 1)
	{
		str = OPS_GetString();
		curArg++;
		if (strcmp(str , "-Damage") == 0 || strcmp(str , "-damage") == 0)
		{
			if (OPS_GetDoubleInput (&numData, &epsPCFac) != 0) {
				opserr << "WARNING invalid -epsPCFac ";
				opserr << "uniaxialMaterial SPSW03: " << tag << endln;
				return 0;
			}
			curArg++;
	
			if (OPS_GetDoubleInput (&numData, &pstCapEFac) != 0) {
				opserr << "WARNING invalid -pstCapEFac ";
				opserr << "uniaxialMaterial SPSW03: " << tag << endln;
				return 0;
			}
			curArg++;
	
			if (OPS_GetDoubleInput (&numData, &gama) != 0) {
				opserr << "WARNING invalid -gama ";
				opserr << "uniaxialMaterial SPSW03: " << tag << endln;
				return 0;
			}
			curArg++;
	
			if (OPS_GetDoubleInput (&numData, &c) != 0) {
				opserr << "WARNING invalid -c ";
				opserr << "uniaxialMaterial SPSW03: " << tag << endln;
				return 0;
			}
			curArg++;
	
			if (OPS_GetDoubleInput (&numData, &resFac) != 0) {
				opserr << "WARNING invalid -resFac ";
				opserr << "uniaxialMaterial SPSW03: " << tag << endln;
				return 0;
			}
			curArg++;
		}
	}

	mode = 1;
	impf = 1000.0;
	if (argc >= curArg + 1)
	{
		str = OPS_GetString();
		curArg++;
		if (strcmp(str, "-IPF") == 0)
		{
			if (paramsSet == true) {
				opserr << "WARNING -IPF is only available for -geom ";
				opserr << "uniaxialMaterial SPSW03: " << tag << endln;
			}
			if (OPS_GetIntInput(&numData, &mode) != 0) {
				opserr << "WARNING invalid -mode ";
				opserr << "uniaxialMaterial SPSW03: " << tag << endln;
				return 0;
			}
			else {
				if ((mode != 1) && (mode != 2)) {
					opserr << "WARNING mode 1 or 2 should be used ";
					opserr << "uniaxialMaterial SPSW03: " << tag << endln;
					return 0;
				}
			}
			curArg++;

			if (OPS_GetDoubleInput(&numData, &impf) != 0) {
				opserr << "WARNING invalid -impf ";
				opserr << "uniaxialMaterial SPSW03: " << tag << endln;
				return 0;
			}
			curArg++;

		}
	}
	if (paramsSet)
		theMaterial = new SPSW03(tag, E0,  b, Fts, Fcs, cmpUnldngEFac, sigTEfac, sigTFfac, epsTFfac, R, epsPCFac, pstCapEFac, gama, c, resFac);
	else
		theMaterial = new SPSW03(tag, fpy, nFac, E0,  b, t, hs, l, R, epsPCFac, pstCapEFac, gama, c, resFac, mode, impf);

	return theMaterial;
}


SPSW03::SPSW03(int tag, double _fpy, double _nFac, double _E0, double _b, double _t, double _hs,
		double _l, double _R, double _epsPCFac, double _pstCapEFac, double _gama, double _c, double _resFac,
		int _mode, double _impf):
		UniaxialMaterial(tag, MAT_TAG_SPSW03), fpy(_fpy), nFac(_nFac), E0(_E0), b(_b), t(_t), hs(_hs), l(_l),
		R(_R), epsPCFac(_epsPCFac), pstcpEFac(_pstCapEFac), gama(_gama), c(_c), resFac(_resFac),
		mode(_mode), impf(_impf)
{
	// imperfection
	lStrip = sqrt(hs * hs + l * l);
	outDeform = lStrip / impf;
	if (mode == 1) {
		lImpf = 2 * sqrt((lStrip / 2.0) * (lStrip / 2.0) + outDeform * outDeform);
	}
	else
	{
		lImpf = 4 * sqrt((lStrip / 4.0) * (lStrip / 4.0) + outDeform * outDeform);
	}
	epsImpf = (lImpf - lStrip) / lStrip;
	E0Impf = 1 / (1 / E0 + epsImpf / fpy);

	givenParams = false;
	excurEnerg = totalEnerg = beta = 0;
	excurEnergP = totalEnergP = betaP = 0;
	cmpUnldngEFac = 0.2;
	sigTEfac = 0.5;						//to be multiplied by Fcs
	sigTFfac = 0.2;						//to be multiplied by Fts
	epsTFfac = 0.5;
	this->Calc_sigcr();
	FTS = Fts;
	FCS = Fcs;
	//FailEnerg = gama * Fts * Fts / E0;
	//epsmaxP	=	Fts / E0	;
	FailEnerg = gama * Fts * Fts / E0Impf;
	epsmaxP = Fts / E0Impf;
	sigmaxP	=	Fts	;
	epss0P	=	0.0	;
	sigs0P	=	0.0	;
	epssrP	=	0.0	;
	sigsrP	=	0.0	;
	epsTFP	=	0.0	;
	plstrP	=	0.0	;
	konP	=	0	;
	eP		=	0.0	;
	sigP	=	0.0	;
	epsP	=	0.0	;
	sig		=	0.0	;
	eps		=	0.0	;
	e		=	0.0	;
}

SPSW03::SPSW03(int tag, double _E0, double _b, double _FTS, double _FCS, double _cmpUnldngEFac,
		double _sigTEfac, double _sigTFfac, double _epsTFfac, double _R, double _epsPCFac,
		double _pstCapEFac, double _gama, double _c, double _resFac):
		UniaxialMaterial(tag, MAT_TAG_SPSW03), E0(_E0), b(_b), FTS(_FTS), FCS(_FCS), cmpUnldngEFac(_cmpUnldngEFac),
		sigTEfac(_sigTEfac), sigTFfac(_sigTFfac), epsTFfac(_epsTFfac),
		R(_R), epsPCFac(_epsPCFac), pstcpEFac(_pstCapEFac), gama(_gama), c(_c), resFac(_resFac)
{
	E0Impf = E0;
	givenParams = true;
	excurEnerg = totalEnerg = beta = 0;
	excurEnergP = totalEnergP = betaP = 0;
	Fts = FTS;
	Fcs = FCS;
	//FailEnerg = gama * Fts * Fts / E0;
	//epsmaxP	=	Fts / E0	;
	FailEnerg = gama * Fts * Fts / E0Impf;
	epsmaxP = Fts / E0Impf;
	sigmaxP	=	Fts	;
	epss0P	=	0.0	;
	sigs0P	=	0.0	;
	epssrP	=	0.0	;
	sigsrP	=	0.0	;
	epsTFP	=	0.0	;
	plstrP	=	0.0	;
	konP	=	0	;
	eP		=	0.0	;
	sigP	=	0.0	;
	epsP	=	0.0	;
	sig		=	0.0	;
	eps		=	0.0	;
	e		=	0.0	;
}

SPSW03::SPSW03():
	UniaxialMaterial(0, MAT_TAG_SPSW03)
{
	konP = 0;
}

SPSW03::~SPSW03()
{

}

UniaxialMaterial* SPSW03::getCopy()
{
	SPSW03* theCopy;
	if (givenParams)
		theCopy = new SPSW03(this->getTag(), E0, b, FTS, FCS, cmpUnldngEFac, sigTEfac, sigTFfac, epsTFfac, R, epsPCFac, pstcpEFac, gama, c, resFac);
	else
		theCopy = new SPSW03(this->getTag(), fpy, nFac, E0, b, t, hs, l, R, epsPCFac, pstcpEFac, gama, c, resFac, mode, impf);
	return theCopy;
}

double SPSW03::getInitialTangent()
{
	return E0;
}

int SPSW03::setTrialStrain(double trialStrain, double strainRate)
{
	double resF = resFac * FTS;
	double Esh = b * E0;
	double epsyn = FCS / E0; // compression yield strain
	//double epsyp = FTS / E0; // tention yield strain
	double epsyp = FTS / E0Impf; // tention yield strain
	eps = trialStrain;
	double deps = eps - epsP;
	epsmax = epsmaxP;
	sigmax = sigmaxP;
	epss0 = epss0P;
	sigs0 = sigs0P;
	epsr = epssrP;
	sigr = sigsrP;
	epsTF = epsTFP;
	plstr = plstrP;
	kon = konP;
	double sigTF = sigTFfac*sigmax;			//sig at tension field redevelopment point
	double epsPC = epsPCFac * epsyp;
	//opserr << "E0Impf at setTrialStrain= " << E0Impf << endln;

	if (kon == 0)
	{
		if (fabs(eps) <= epsyn)
		{
			//sig = eps * E0;
			sig = eps * E0Impf;
			e = E0Impf;
		}
		else if (eps < -epsyn)
		{
			if (deps > 0)
				kon = 11;
			else
			{
				sig = -Fcs;
				e = 1.0e-15*E0;
			}
		}
		else if (eps > epsyn)
		{
			kon = 21;
			epsr = 0.0;
			sigr = 0.0;
			epss0 = epsyp;
			sigs0 = Fts;
		}
	}
	
	//kon == 11: comp. branch
	if (kon == 11)
	{
		if (deps > 0)
		{
			//if (epsP > plstr - 2.*Fcs/(cmpUnldngEFac*E0))
			if (epsP > plstr - 2. * Fcs / (cmpUnldngEFac * E0Impf))
				kon = 12;
			else
			{
				kon = 13;
				//epsTF = plstr - epsTFfac*(plstr - (epsP + Fcs / (cmpUnldngEFac*E0))) + sigTF / E0;
				epsTF = plstr - epsTFfac * (plstr - (epsP + Fcs / (cmpUnldngEFac * E0Impf))) + sigTF / E0Impf;
				epsr = epsP;
				sigr = sigP;
				//epss0 = epsr - (sigr - sigTEfac*Fcs) / (cmpUnldngEFac*E0);
				epss0 = epsr - (sigr - sigTEfac * Fcs) / (cmpUnldngEFac * E0Impf);
				sigs0 = sigTEfac*Fcs;
			}
		}
		else
		{
			sig = -Fcs;
			e = 1.0e-15*E0;
		}
	}

	// compression reloading branch
	if (kon == 12)
	{
		//sig = sigP + cmpUnldngEFac*E0*deps;
		//e = cmpUnldngEFac*E0;
		//double sigUnloading = E0 * (eps - plstr);
		sig = sigP + cmpUnldngEFac * E0Impf * deps;
		e = cmpUnldngEFac * E0Impf;
		double sigUnloading = E0Impf * (eps - plstr);
		if (sig <= -Fcs)
		{
			kon = 11;
			sig = -Fcs;
			e = 1.0e-15*E0;
		}
		else if (sig <= sigUnloading)		// check to see if reloading branch coincides with the previous unloading branch
		{
			kon = 21;
			sig = sigUnloading;
			//e= E0;
			e = E0Impf;
			epsr = eps;
			sigr = sig;
			double ET = (Fts - sigr) / (epsmax - epsr);
			epss0 = (Fts - Esh * epsyp - sigr + ET * epsr) / (ET - Esh);
			sigs0 = Fts + Esh * (epss0 - epsyp);
			//this->MenegottoPinto(eps, Esh, R, sig, e);
			return 0;
		}
	}
	if (kon == 13)
	{
		if (eps > epsTF)
		{
			kon = 21;
			epsr = epsTF;
			sigr = sigTF;
			double ET = (Fts - sigr) / (epsmax - epsr);
			epss0 = (Fts - Esh * epsyp - sigr + ET * epsr) / (ET - Esh);
			sigs0 = Fts + Esh * (epss0 - epsyp);
			//this->MenegottoPinto(eps, Esh, R, sig, e);
		}
		else
		{
			double bT = (sigTF - sigs0) / (epsTF - epss0)/*/(0.2*E0)*/;
			double sigEnvlp, eEnvlp;
			this->MenegottoPinto(eps, bT, R, sigEnvlp, eEnvlp);
			//sig = sigP + cmpUnldngEFac*E0*deps;
			//e = cmpUnldngEFac*E0;
			sig = sigP + cmpUnldngEFac * E0Impf * deps;
			e = cmpUnldngEFac * E0Impf;
			if (sig <= -Fcs)
			{
				kon = 11; sig = -Fcs;
				e = 1.0e-15*E0;
			}
			//commented to handle non-convergence ny using the same branch in loading and unloading directions:
			else /*if (sig >= sigEnvlp)*/
			{
				sig = sigEnvlp;
				e = eEnvlp;
			}
		}
	}

	// tension branch
	if (kon == 21)
	{
		if (epsP > epsmax)
		{
			sigmax = sigP;
			epsmax = epsP;
		}
		if (epsmax > epsPC)
		{
			if (deps > 0)
			{
				
				if (eps < epsmax)
				{
					e = (sigmax - sigr) / (epsmax - epsr);
					sig = sigP + e * deps;
				}
				else
				{
					//e = pstcpEFac * E0;
					e = pstcpEFac * E0Impf;
					sig = sigP + e * deps;
					if (sig < resF)
					{
						sig = resF;
						e = 1.0e-15*E0;
					}
				}
			}
			else
			{
				//sig = sigP + E0 * deps;
				sig = sigP + E0Impf * deps;
				epsr = eps;
				sigr = sig;
				//e = E0;
				e = E0Impf;
				if (sig <= -Fcs)
				{
					kon = 11;
					sig = -Fcs;
					e = 1.0e-15*E0;
					//plstr = epsP - sigP/E0;
					plstr = epsP - sigP / E0Impf;
				}
			}
		}
		else
		{
			double sigEnvlp, eEnvlp;
			this->MenegottoPinto(eps, Esh, R, sigEnvlp, eEnvlp);
			sig = sigP + E0 * deps;
			e = E0;
			//sig = sigP + E0Impf * deps;
			//e = E0Impf;
			if (sig <= -Fcs)
			{
				kon = 11;
				sig = -Fcs;
				e = 1.0e-15*E0;
				plstr = epsP - sigP/E0;
				//plstr = epsP - sigP / E0Impf;
			}
			else if (sig >= sigEnvlp)
			{
				sig = sigEnvlp;
				e = eEnvlp;
			}
		}
	}
	return 0;
	
}

void SPSW03::updateDamage()
{
	if ( (sig < 0.0 && sigP >= 0) || (sig == 0 && sigP > 0))
	{
		//submit damage and reset for new excursion
		//double zeroSigEps = epsP - sigP/E0;
		double zeroSigEps = epsP - sigP / E0Impf;
		double dE = 0.5 * sigP * (zeroSigEps - epsP);
		totalEnerg += dE;
		if (totalEnerg < 0) totalEnerg = 0.;
		if (gama > 9999)
		{
			return;
		}
		excurEnerg += dE;
		if (excurEnerg < 0) excurEnerg = 0.;
		beta = pow( excurEnerg / ( FailEnerg - totalEnerg) , c );
		if (beta > 0.999 || beta < 0)
		{
			opserr<< "\nSPSW03:"<< this->getTag()<< " WARNING! Maximum Energy Absorbance Capacity Reached\n"<< endln;
			beta = 0.999;

		}
		sigmaxP = (1. - beta)*sigmaxP + beta * resFac*FTS;
		Fts = (1. - beta)*Fts + beta * resFac*FTS;
		if (Fcs > Fts)
			Fcs = Fts;
		excurEnerg = 0.0;
	} 
	else if ( sig > 0.0 )
	{
		double dE = 0.5 * (sig + sigP) * (eps - epsP);
		excurEnerg += dE;
		totalEnerg += dE;
    }
}

double SPSW03::getStrain()
{
	return eps;
}

double SPSW03::getStress()
{
	return sig;
}

double SPSW03::getTangent()
{
	return e;
}

int SPSW03::commitState()
{
	epsmaxP = epsmax;
	sigmaxP = sigmax;
	epss0P = epss0;
	sigs0P = sigs0;
	epssrP = epsr;
	sigsrP = sigr;
	epsTFP = epsTF;
	plstrP = plstr;
	konP = kon;

	this->updateDamage();
	eP = e;
	sigP = sig;
	epsP = eps;
	totalEnergP = totalEnerg;
	excurEnergP = excurEnerg;
	return 0;
}

int SPSW03::revertToLastCommit()
{
	epsmax	=	epsmaxP	;
	sigmax	=	sigmaxP	;
	epss0	=	epss0P	;
	sigs0	=	sigs0P	;
	epsr	=	epssrP	;
	sigr	=	sigsrP	;
	epsTF	=	epsTFP	;
	plstr	=	plstrP	;
	kon		=	konP	;
	e		=	eP		;
	sig		=	sigP	;
	eps		=	epsP	;
	excurEnerg = excurEnergP;
	totalEnerg = totalEnergP;
	beta = betaP;
	
	return 0;
}

int SPSW03::revertToStart()
{
	opserr <<"revert called\n";
	excurEnerg = totalEnerg = beta = 0;
	excurEnergP = totalEnergP = betaP = 0;
	Fts = FTS;
	Fcs = FCS;
	if (!givenParams)
	{
		this->Calc_sigcr();
		FTS = Fts;
		FCS = Fcs;
	}
	FailEnerg = gama * Fts * Fts/E0;
	epsmaxP	=	Fts / E0	;
	//FailEnerg = gama * Fts * Fts / E0Impf;
	//epsmaxP = Fts / E0Impf;
	sigmaxP	=	Fts	;
	epss0P	=	0.0	;
	sigs0P	=	0.0	;
	epssrP	=	0.0	;
	sigsrP	=	0.0	;
	epsTFP	=	0.0	;
	plstrP	=	0.0	;
	konP	=	0	;
	eP		=	0.0	;
	sigP	=	0.0	;
	epsP	=	0.0	;
	sig		=	0.0	;
	eps		=	0.0	;
	e		=	0.0	;
	lStrip = sqrt(hs * hs + l * l);
	lImpf = 1.01 * lStrip;
	epsImpf = (lImpf - lStrip) / lStrip;
	E0Impf = 1 / (1 / E0 + epsImpf / fpy);
	opserr << "E0Impf at revertToStart = " << E0Impf << endln;
	return 0;
}

int SPSW03::sendSelf(int commitTag, Channel & theChannel)
{
				
	int res = 0;
	static Vector data(42);
	data(0)  = this->getTag();
	data(1)  = t			;
	data(2)  = hs			;
	data(3)  = l			;
	data(4)  = fpy			;
	data(5)  = E0			;
	data(6)  = b			;
	data(7)  = R			;
	data(8)  = Fts			;
	data(9)  = Fcs			;
	data(10) = FTS			;
	data(11) = FCS			;
	data(12) = epsPCFac		;
	data(13) = pstcpEFac	;
	data(14) = gama			;
	data(15) = FailEnerg	;
	data(16) = c			;
	data(17) = resFac		;
	data(18) = mode         ;
	data(19) = impf         ;
	data(20) = givenParams	;
	data(21) = cmpUnldngEFac;
	data(22) = sigTEfac		;
	data(23) = sigTFfac		;
	data(24) = epsTFfac		;
	data(25) = epsmaxP		;
	data(26) = sigmaxP		;
	data(27) = epss0P		;
	data(28) = sigs0P		;
	data(29) = epssrP		;
	data(30) = sigsrP		;
	data(31) = epsTFP		;
	data(32) = plstrP		;
	data(33) = konP			;
	data(34) = epsP			;
	data(35) = sigP			;
	data(36) = eP			;
	data(37) = excurEnergP	;
	data(38) = totalEnergP	;
	data(39) = betaP		;
	data(40) = E0Impf		;
	data(41) = nFac			;
	
	// Data is only sent after convergence, so no trial variables
	// need to be sent through data vector

	res = theChannel.sendVector(this->getDbTag(), commitTag, data);
	if (res < 0)
		opserr << "SPSW03::sendSelf() - failed to send data\n";

	return res;
}

int SPSW03::recvSelf(int commitTag, Channel & theChannel, FEM_ObjectBroker & theBroker)
{
	int res = 0;
	static Vector data(42);
	res = theChannel.recvVector(this->getDbTag(), commitTag, data);

	if (res < 0) {
		opserr << "SPSW03::recvSelf() - failed to receive data\n";
		this->setTag(0);
		return res;
	}
	this->setTag(int(data(0)));
	t = data(1)				;
	hs = data(2)			;
	l = data(3)				;
	fpy = data(4)			;
	E0 = data(5)			;
	b = data(6)				;
	R = data(7)				;
	Fts = data(8)			;
	Fcs = data(9)			;
	FTS = data(10)			;
	FCS = data(11)			;
	epsPCFac = data(12)		;
	pstcpEFac = data(13)	;
	gama = data(14)			;
	FailEnerg = data(15)	;
	c = data(16)			;
	resFac = data(17)		;
	mode = data(18)			;
	impf = data(19)			;
	givenParams = data(20)	;
	cmpUnldngEFac = data(21);
	sigTEfac = data(22)		;
	sigTFfac = data(23)		;
	epsTFfac = data(24)		;
	epsmaxP = data(25)		;
	sigmaxP = data(26)		;
	epss0P = data(27)		;
	sigs0P = data(28)		;
	epssrP = data(29)		;
	sigsrP = data(30)		;
	epsTFP = data(31)		;
	plstrP = data(32)		;
	konP = data(33)			;
	epsP = data(34)			;
	sigP = data(35)			;
	eP = data(36)			;
	excurEnergP = data(37)	;
	totalEnergP = data(38)	;
	betaP = data(39)		;
	E0Impf = data(40)       ;
	nFac = data(41);
	epsmax = epsmaxP		;
	sigmax = sigmaxP		;
	epss0 = epss0P			;
	sigs0 = sigs0P			;
	epsr = epssrP			;
	sigr = sigsrP			;
	epsTF = epsTFP			;
	plstr = plstrP			;
	kon = konP				;
	eps = epsP				;
	sig = sigP				;
	e = eP					;
	excurEnerg = excurEnergP;
	totalEnerg = totalEnergP;
	beta = betaP			;
}

void SPSW03::Print(OPS_Stream & s, int flag)
{
	s << "SPSW03:(strain, stress, tangent)" << eps << " " << sig << " " << e << endln;
}

void SPSW03::Calc_sigcr()
{
	// calculate plate buckling strength (Fcs) and tension strength (Fts)
	double hsToL = hs / l;
	double ks = 5.6 + 8.98 / hsToL / hsToL;
	if (hsToL > 1.0)
		ks = 8.98 + 5.6 / hsToL / hsToL;
	double pi = 2 * asin(1.0);
	double mu = 0.3;
	double lToT = l / t;
	// Fcs = ks * pi * pi * E0 / (12 * (1.0 - mu * mu) * lToT * lToT); // buickling stress
	Fcs = nFac * fpy;
	Fts = pow(fpy * fpy - 0.75 * Fcs * Fcs, 0.5) - 0.5 * Fcs;    // yield strength of tensional strip acording to Von-Mises rule
	return;
}

void SPSW03::MenegottoPinto(double epsc, double E2, double R, double & sigc, double& ec)
{
	double E1 = (sigs0 - sigr) / (epss0 - epsr);
	double bT = E2/E1;
	// double bT = E2;
	double epsrat = (epsc-epsr) / (epss0 - epsr);
	double dum1 = 1.0 + pow (fabs(epsrat),R);
	double dum2 = pow(dum1, 1.0/R);
	sigc = bT * epsrat + (1.0 - bT) * epsrat / dum2;
	sigc = sigc * (sigs0 - sigr) + sigr;
	ec = bT + (1.0 - bT) / (dum1 * dum2);
	ec *= (sigs0 - sigr) / (epss0 - epsr);
}

double SPSW03::getEnergy()
{
	return totalEnergP;
}

//Response* 
//SPSW03::setResponse(const char **argv, int argc,
//			      OPS_Stream &theOutput)
//{
//	Response *theResponse = UniaxialMaterial::setResponse(argv, argc, theOutput);
//	if (theResponse != 0)
//		return theResponse;
//
//	if ( (strcmp(argv[0],"energy") == 0) ||
//		(strcmp(argv[0],"Energy") == 0) ) {
//    
//		theOutput.tag("UniaxialMaterialOutput");
//		theOutput.attr("matType", this->getClassType());
//		theOutput.attr("matTag", this->getTag());
//    
//		theOutput.tag("ResponseType", "energy");
//		theResponse =  new MaterialResponse(this, 6, totalEnergP);
//		theOutput.endTag();
//  }
//  
//  return theResponse;
//
//}
// 
//int 
//SPSW03::getResponse(int responseID, Information &matInfo)
//{
//	if (UniaxialMaterial::getResponse(responseID, matInfo) == 0)
//		return 0;
//	//opserr << "getResponse-2\n";
//	switch (responseID) {
//	case 6:
//		matInfo.setDouble(totalEnergP);
//		return 0;
//	default:      
//		return -1;
//	}
//}
//
